...
int main (int argc, char *argv[])
{
    ...
    MPE_Open_graphics(&graph, world, NULL, -1, -1, 
                       WINDOW_SIZE, WINDOW_SIZE, 0);
    
    // let the master process draw the circle
    // MPE_Draw_circle(graph, (int)WINDOW_SIZE/2, (int)WINDOW_SIZE/2, 
    //                 (int)WINDOW_SIZE/2, MPE_BLACK);
    // MPE_Update(graph);
        ...
        while (!done)
        {
            ...
            for (i = 0; i < CHUNKSIZE-1;)
            {
                ...
                if(x*x + y*y < 1.0)
                {
                    in++;
                    #if QUARTER_CIRCLE
                    MPE_Draw_point(graph, (int)(WINDOW_SIZE*x), 
                                    (int)(WINDOW_SIZE - y*WINDOW_SIZE), MPE_BLACK);
                    #else
                    MPE_Draw_point(graph, (int)(WINDOW_SIZE/2 + x * WINDOW_SIZE/2), 
                                    (int)(WINDOW_SIZE/2 - y*WINDOW_SIZE/2), MPE_BLACK);
                    #endif
                    MPE_Update(graph);
                }
                ...
                else
                {
                    ...
                    #if QUARTER_CIRCLE
                    MPE_Draw_point(graph, (int)(WINDOW_SIZE*x), 
                                    (int)(WINDOW_SIZE - y*WINDOW_SIZE), MPE_YELLOW);
                    #else
                    MPE_Draw_point(graph, (int)(WINDOW_SIZE/2 + x * WINDOW_SIZE/2), 
                                    (int)(WINDOW_SIZE/2 - y*WINDOW_SIZE/2), MPE_YELLOW);
                    #endif
                    MPE_Update(graph);
                }
            }
